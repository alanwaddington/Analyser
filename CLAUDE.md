...
- **Stryd-first power precedence (PR #117):** When a FIT file contains both the Stryd developer field `Power` (capital P) and the native watch `power` field, the parser uses `r['Power'] ?? r.power` — Stryd takes priority. This resolves the ambiguity for paired Garmin+Stryd setups where Garmin native power also writes to the standard field.
- **Power source detection (PR #117):** After normalisation, `Activity.powerSource` is set: `'stryd'` when any record contained the capital-P developer field; `'native'` when native power exists on a running activity (no Stryd); `'cycling'` when native power exists on a cycling/unknown-sport activity; `undefined` when no power data is present. Note: there is **no** `strydPower` ChannelKey — both Stryd and native power are stored in the single `power` channel, with source attribution carried on `Activity.powerSource`.
- **Manufacturer-aware device labels (PR #117):** `deriveDeviceLabel()` in `deviceChannels.ts` now accepts an optional stream and activity. For power-contributing devices in running activities, it returns source-specific labels: `'Stryd'` (when `powerSource === 'stryd'`), `'[Manufacturer] Running Power'` (when `powerSource === 'native'`, e.g. `'Garmin Running Power'`). Cycling power devices retain their standard manufacturer+product label. `MANUFACTURER_NAMES` in `deviceChannels.ts` maps known manufacturer strings to display names (Garmin, Suunto, COROS, Polar, Wahoo).
